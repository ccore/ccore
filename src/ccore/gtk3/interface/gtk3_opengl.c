...
static bool _needsRender = false;
...
static gboolean eventConfigure(GtkWidget *widget, GdkEventConfigure *event, gpointer data)
{
	...
	_cSurface = gdk_window_create_similar_surface(gtk_widget_get_window(widget), CAIRO_CONTENT_COLOR, w, h);

	// Clear cairo surface
	cairo_t *cr = cairo_create(_cSurface);
...
void ccGtkContextSetCurrent(void)
{
	if(_gContext == NULL){
		return;
	}
...
ccError ccGLContextBind(void)
{
	...
	g_signal_connect(_gWidget, "configure-event", G_CALLBACK(eventConfigure), NULL);
	g_signal_connect(_gWidget, "draw", G_CALLBACK(eventDraw), NULL);

	gtk_container_add(GTK_CONTAINER(win), _gWidget);
	gtk_widget_show_all(win);

	GdkWindow *gdkw = gtk_widget_get_window(_gWidget);
